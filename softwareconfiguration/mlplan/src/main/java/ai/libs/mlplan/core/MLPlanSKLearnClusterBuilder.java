...
public class MLPlanSKLearnClusterBuilder extends MLPlanSKLearnBuilder {
	...
	private final Logger logger = LoggerFactory.getLogger(MLPlanSKLearnClusterBuilder.class);
	 ...
	 */
	MLPlanSKLearnClusterBuilder() throws IOException {
		this(false);
	}
	 ...
	 * Creates a new ML-Plan Builder for scikit-learn.
	 *
	 * @param skipSetupCheck Flag whether to skip the system's setup check, which examines whether the operating system has python installed in the required version and all the required python modules
	 * are installed.
	 * @throws IOException Thrown if configuration files cannot be read.
	 */
	private MLPlanSKLearnClusterBuilder(final boolean skipSetupCheck) throws IOException {
		super();
		if (!skipSetupCheck) {
	...
	public MLPlanSKLearnClusterBuilder withValidationMeasure(final IMeasure<Instances, Double> measure) {
